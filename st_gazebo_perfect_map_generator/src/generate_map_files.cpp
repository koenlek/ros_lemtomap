...
bool createVectorArray(const char * vectorString,
	...
	for (it = corners.begin(); it != corners.end(); ++it) {
		...
		if (opening == string::npos || closing == string::npos) {
			ROS_INFO_STREAM( "Poorly formed string: " << cornersStr );
			ROS_INFO_STREAM( "( found at: " << opening << " ) found at: " << closing);
			return false;
		}
		string oneCornerStr = cornersStr.substr(opening + 1,
				closing - opening - 1);
		size_t commaLoc = oneCornerStr.find(",");
		string x = oneCornerStr.substr(0, commaLoc);
		string y = oneCornerStr.substr(commaLoc + 1,
				oneCornerStr.length() - commaLoc);
...
int main(int argc, char * argv[]) {
	if (argc > 4) {
		...
		ROS_INFO_STREAM( "Request: " << " UL.x: " << request.upperleft().x()
				<< " UL.y: " << request.upperleft().y() << " UR.x: "
				<< request.upperright().x() << " UR.y: "
				<< request.upperright().y() << " LR.x: "
				<< request.lowerright().x() << " LR.y: "
				<< request.lowerright().y() << " LL.x: "
				<< request.lowerleft().x() << " LL.y: "
				<< request.lowerleft().y() << " Height: " << request.height()
				<< " Resolution: " << request.resolution() << " Filename: "
				<< request.filename() << " Threshold: " << request.threshold()
				);

		gazebo::transport::PublisherPtr imagePub = node->Advertise<
