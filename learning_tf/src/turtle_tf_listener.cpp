...
int main(int argc, char** argv) {
	...
	while (node.ok()) {
		...
		try {
			...
			ros::Time past = now - ros::Duration(5.0); //KL: instead of now, ros::Time::now() could also be used, then this line could be removed
			...
			ROS_ERROR("%s", ex.what());
			//std::cout<<"hoi"<<std::endl; //KL does not work?
		}
